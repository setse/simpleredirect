...
export async function setVoronoiCellData(voronoiId, cellId, data) {
  const db = getRedis();
  if (!data || (!data.url && !data.name && !data.redirectUrl)) {
...
export async function hasVoronoiCellFile(voronoiId, cellId) {
  const db = getRedis();
  return await db.exists(`voronoi:${voronoiId}:cell:${cellId}:file`);
}
